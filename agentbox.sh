...
function cmd_start() {
    ...
    if [[ -n "${custom_image}" ]]; then
        ...
        has_node=0
        image_has_node "${cmd}" "${user_image_ref}" && has_node=1
        has_bash=0
        image_has_command "${cmd}" "${user_image_ref}" bash && has_bash=1
        has_base64=0
        image_has_command "${cmd}" "${user_image_ref}" base64 && has_base64=1
